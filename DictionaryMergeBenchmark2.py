...
def benchmark_dict_merge():
    ...
    print(f"Dictionary union operator (|) support: {'Yes' if has_union_support else 'No'}")
    print("=" * 50)
    ...
    for size in sizes:
        ...
        if has_union_support:
            ...
            print("  Union operator not available in this Python version")
    ...
    if has_union_support:
        ...
        print("Only unpacking method benchmarked (union operator not supported)")
        for i, size in enumerate(sizes):
            time_per_op = results['unpacking_times'][i] / iterations * 1000
            print(f"Size {size:5d}: {time_per_op:.3f}ms per merge operation")
...
def benchmark_three_dicts():
    ...
    print("\n" + "=" * 50)
    print("BONUS: Three Dictionary Merge")
    print("=" * 50)
