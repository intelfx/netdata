...
#     restricted          Just chmod 0750 (group-executable, no elevation).
#
#   If <caps> contains multiple arguments, these are treated as alternatives
#   and are tried left to right (e.g. "cap_perfmon+ep" "cap_sys_admin+ep").
# =========================================================================
...
foreach_privileged_plugin() {
  ...
  "${_fp_cb}" debugfs.plugin                     caps-verify  "cap_dac_read_search,cap_audit_control+ep"
  ...
  "${_fp_cb}" go.d.plugin                        caps         "cap_dac_read_search+epi cap_net_admin+epi cap_net_raw=eip cap_net_bind_service=eip"
  "${_fp_cb}" macos-logs.plugin                  setuid
  ...
  "${_fp_cb}" cgroup-name                        restricted
  "${_fp_cb}" snmp-trap-profile-gen              restricted
  "${_fp_cb}" otel-plugin                        restricted
  "${_fp_cb}" netflow-plugin                     restricted
}
...
if [ -n "${NETDATA_DESTDIR}" ]; then
  # ---------------------------------------------------------------------------
  progress "Generate post-install script for DESTDIR deployment"

  _deploy_script="${NETDATA_PREFIX}/usr/libexec/netdata/netdata-post-install.sh"

  # Callback: emit a set_privs call for each privileged plugin that was built.
  # shellcheck disable=SC2329
  _emit_set_privs() {
    _esp_name="${1}"; _esp_mode="${2}"; shift 2
    [ ! -f "${NETDATA_PREFIX}/usr/libexec/netdata/plugins.d/${_esp_name}" ] && return 0
    printf 'set_privs "%s" "%s"' "${_esp_name}" "${_esp_mode}"
    for _esp_cap; do
      printf ' "%s"' "${_esp_cap}"
    done
    printf '\n'
  ...
  # --- Part 1: header with values expanded at generation time ---
  cat > "${_deploy_script}" << DEPLOY_HEAD
#!/bin/sh
# netdata-post-install.sh
# Generated by netdata-installer.sh --destdir on $(date -u +%Y-%m-%dT%H:%M:%SZ).
#
# Run as root on the target system after unpacking the DESTDIR archive onto /.
# Creates the netdata user/group, sets file ownership, and configures plugin
# privileges (Linux capabilities with setuid-root fallback).
#
# Usage: netdata-post-install.sh [--no-start]
...
_admin_group=""
if command -v getent >/dev/null 2>&1; then
  getent group root   >/dev/null 2>&1 && _admin_group="root"
  [ -z "${_admin_group}" ] && getent group daemon >/dev/null 2>&1 && _admin_group="daemon"
else
  grep -q "^root:"   /etc/group 2>/dev/null && _admin_group="root"
  [ -z "${_admin_group}" ] && grep -q "^daemon:" /etc/group 2>/dev/null && _admin_group="daemon"
fi
[ -z "${_admin_group}" ] && _admin_group="${NETDATA_GROUP}"

chown "${NETDATA_USER}:${_admin_group}" "${PREFIX}/var/log/netdata"
chown -R "root:${_admin_group}" "${LIBEXEC}"
find "${LIBEXEC}" -type d -exec chmod 0755 {} \;
find "${LIBEXEC}" -type f -exec chmod 0644 {} \;
find "${LIBEXEC}" -type f -name '*plugin' -exec chown ":${NETDATA_GROUP}" {} \;
find "${LIBEXEC}" -type f -name '*plugin' -exec chmod 0750 {} \;
find "${LIBEXEC}" -type f -name '*.sh'    -exec chmod 0755 {} \;

# =========================================================================
echo "==> Setting plugin privileges"
# =========================================================================
...
# set_privs <name> <mode> [<cap> ...]
#
# Modes:
#   caps-verify  Try setcap, verify with -t, fall back to setuid.
#   caps         Try setcap, fall back to setuid.
#   setuid       Always setuid root.
#   restricted   Just 0750 (group-executable).
set_privs() {
  _sp_name="${1}"; _sp_mode="${2}"; shift 2
  _sp_path="${PLUGDIR}/${_sp_name}"
  [ ! -f "${_sp_path}" ] && return 0

  chown "root:${NETDATA_GROUP}" "${_sp_path}"

  case "${_sp_mode}" in
    caps|caps-verify)
      if ! _in_container && command -v setcap >/dev/null 2>&1; then
        chmod 0750 "${_sp_path}"
        for _sp_cap; do
          if setcap "${_sp_cap}" "${_sp_path}" 2>/dev/null; then
            if [ "${_sp_mode}" = "caps-verify" ]; then
              "${_sp_path}" -t >/dev/null 2>&1 || continue
            fi
            echo "  ${_sp_name}: capabilities OK (${_sp_cap})"
            return
          fi
        done
      fi
      chmod 4750 "${_sp_path}"
      echo "  ${_sp_name}: setuid root (capabilities unavailable)"
      ;;
    setuid)
      chmod 4750 "${_sp_path}"
      echo "  ${_sp_name}: setuid root"
      ;;
    restricted)
      chmod 0750 "${_sp_path}"
      echo "  ${_sp_name}: restricted (0750)"
      ;;
  esac
}
...
DEPLOY_BODY
  ...
  # --- Part 3: plugin table (expanded from foreach at generation time) ---
  foreach_privileged_plugin _emit_set_privs >> "${_deploy_script}"
...
DEPLOY_FOOT
  ...
  chmod 0755 "${_deploy_script}"
  echo >&2
  echo >&2 "Post-install script written to: ${TPUT_GREEN}${TPUT_BOLD}${_deploy_script}${TPUT_RESET}"
  echo >&2 "Run it as root on the target system after unpacking the archive onto /."
  echo >&2

elif [ -f "${NETDATA_PREFIX}/usr/libexec/netdata/plugins.d/apps.plugin" ]; then
  # ---------------------------------------------------------------------------
